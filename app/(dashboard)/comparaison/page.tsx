...
const COMP_PART_COLORS = ['#12b76a','#3b6cf8','#61DAFB','#a78bfa','#34d399'];
const CURRENT_YEAR     = new Date().getFullYear();

const bestMonthStr = (idx: number, val: number) =>
  idx >= 0 && val > 0 ? `${MOIS_LABELS[idx]} · ${val.toFixed(0)} TND` : '—';

/* ─── sous-composants stateless module-level ────────────────────── */
function Row({ label, value, color, isMobile }: {
  label: string; value: string | number; color: string; isMobile: boolean;
}) {
  return (
    <div style={{display:'flex',flexDirection:'column',padding:isMobile?'5px 7px':'9px 12px',borderRadius:8,background:`${color}08`,border:`1px solid ${color}25`,gap:1}}>
      <span style={{fontSize:isMobile?9:11,color:'#fff',fontWeight:600,lineHeight:1.2}}>{label}</span>
      <span style={{fontSize:isMobile?11:13,fontWeight:800,color,lineHeight:1}}>
...
function Sep({ label, isMobile }: { label: string; isMobile: boolean }) {
  return (
    <div style={{fontSize:isMobile?8:10,fontWeight:700,color:'#fff',textTransform:'uppercase',letterSpacing:1,marginTop:isMobile?6:10,marginBottom:2,paddingLeft:2}}>
      {label}
    </div>
  );
}
...
interface CompStats {
  ...
  globalScore: number | null; minYear: number;
}

const EMPTY_METRICS: Metrics = { gained:0, paid:0, pending:0, installe:0, annulee:0, commMax:0, commMin:0, payRate:0, total:0 };
...
export default function ComparaisonPage() {
  ...
  /* ── fetch stats avec AbortController ── */
  ...
  const mCurr       = compStats?.metricsA ?? EMPTY_METRICS;
  const mPrev       = compStats?.metricsB ?? EMPTY_METRICS;
  const minYear     = compStats?.minYear ?? CURRENT_YEAR;
  const globalScore = compStats?.globalScore ?? null;
  const hasPrevData = mPrev.total > 0;
  const hasCurrData = mCurr.total > 0;

  const scoreColor = useMemo(() =>
    globalScore === null ? '#8b8b9e' : globalScore > 0 ? '#12b76a' : globalScore < 0 ? '#ef4444' : '#8b8b9e',
  [globalScore]);

  const scoreEmoji = useMemo(() =>
    globalScore === null ? '📊' : globalScore > 20 ? '🚀' : globalScore > 0 ? '📈' : globalScore === 0 ? '➡️' : '📉',
  [globalScore]);
  ...
  /* ── objectifs pré-calculés (évite l'IIFE dans le JSX) ── */
  const objAnnuel = settings.objectifAnnuel as Record<string, number>;
  const objCurr   = objAnnuel?.[String(currYear)] ?? 0;
  const pctCurr   = objCurr > 0 ? Math.round((mCurr.gained / objCurr) * 100) : 0;
  const objPrev   = objAnnuel?.[String(prevYear)] ?? 0;
  const pctPrev   = objPrev > 0 ? Math.round((mPrev.gained / objPrev) * 100) : 0;
  ...
  return (
    <div style={{position:'relative',minHeight:'100vh',color:'#fff',overflow:'hidden'}}>
      ...
      <CosmosBackground particleColors={COMP_PART_COLORS} />
      ...
      <div style={{position:'relative',zIndex:1,padding:isMobile?'16px 12px 40px':'28px 32px 40px'}}>
        ...
        {/* ════════════════════════════════════════
            HEADER
            ════════════════════════════════════════ */}
        <div style={{padding:'1.5px',borderRadius:22,background:'linear-gradient(135deg,#3b82f670,#60a5fa40,#a78bfa25)',marginBottom:24,animation:'fadeSlideUp 0.4s ease both'}}>
          <div style={{background:'rgba(2,8,16,0.97)',borderRadius:'20.5px',padding:isMobile?'20px 16px':'28px 32px',position:'relative',overflow:'hidden',backdropFilter:'blur(40px)'}}>
            ...
            <div style={{position:'relative',display:'flex',flexDirection:isMobile?'column':'row',alignItems:isMobile?'flex-start':'center',justifyContent:'space-between',gap:16}}>
              ...
              <div style={{display:'flex',alignItems:'center',gap:8,flexShrink:0,flexWrap:'wrap'}}>
                ...
                <button onClick={()=>setSelectedYear(y=>y+1)} disabled={selectedYear>=CURRENT_YEAR}
                  style={{width:36,height:36,borderRadius:10,border:`1px solid ${selectedYear>=CURRENT_YEAR?'rgba(255,255,255,0.06)':'rgba(255,255,255,0.14)'}`,background:'transparent',color:selectedYear>=CURRENT_YEAR?'rgba(255,255,255,0.15)':'rgba(255,255,255,0.55)',cursor:selectedYear>=CURRENT_YEAR?'not-allowed':'pointer',display:'flex',alignItems:'center',justifyContent:'center',transition:'all 0.18s'}}
                  onMouseEnter={e=>{if(selectedYear<CURRENT_YEAR){e.currentTarget.style.background='rgba(255,255,255,0.07)';e.currentTarget.style.color='#fff';}}}
                  onMouseLeave={e=>{e.currentTarget.style.background='transparent';e.currentTarget.style.color=selectedYear>=CURRENT_YEAR?'rgba(255,255,255,0.15)':'rgba(255,255,255,0.55)';}}>
                  <ChevronRight size={16}/>
                </button>

                {selectedYear!==CURRENT_YEAR&&(
                  <button onClick={()=>setSelectedYear(CURRENT_YEAR)}
        ...
        {loading?(
          ...
          <>
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:isMobile?8:16,marginBottom:20}}>
              {[
                { year:prevYear, m:mPrev, color:'#3b6cf8', obj:objPrev, pct:pctPrev,
                  bestMth:bestMonthStr(compStats?.bestMonthPrev??-1, compStats?.bestMonthPrevVal??0) },
                { year:currYear, m:mCurr, color:'#12b76a', obj:objCurr, pct:pctCurr,
                  bestMth:bestMonthStr(compStats?.bestMonthCurr??-1, compStats?.bestMonthCurrVal??0) },
              ].map(({year,m,color,obj,pct,bestMth})=>(
                <div key={year} style={{padding:'1.5px',borderRadius:18,background:`linear-gradient(135deg,${color}40,${color}12)`}}>
                  <div style={{background:'rgba(2,8,16,0.97)',borderRadius:'16.5px',padding:isMobile?'10px 8px':'22px',backdropFilter:'blur(20px)'}}>
                    ...
                    <div style={{display:'flex',flexDirection:'column',gap:isMobile?4:6}}>
                      <Sep label="Leads" isMobile={isMobile}/>
                      <Row label="Total fiches"           value={m.total}                                         color="#a78bfa" isMobile={isMobile}/>
                      <Row label="Installations"           value={m.installe}                                      color="#12b76a" isMobile={isMobile}/>
                      <Row label="Installations annulées"  value={m.annulee}                                       color="#ef4444" isMobile={isMobile}/>
                      <Row label="Taux d'installation"    value={m.total>0?`${Math.round((m.installe/m.total)*100)}%`:'0%'} color="#22c55e" isMobile={isMobile}/>
                      <Sep label="Commissions" isMobile={isMobile}/>
                      <Row label="Total commissions"      value={m.gained>0?`${m.gained.toFixed(0)} TND`:'0 TND'}   color="#12b76a" isMobile={isMobile}/>
                      <Row label="Commissions payées"     value={m.paid>0?`${m.paid.toFixed(0)} TND`:'0 TND'}       color="#3b6cf8" isMobile={isMobile}/>
                      <Row label="En attente"             value={m.pending>0?`${m.pending.toFixed(0)} TND`:'0 TND'}  color="#f79009" isMobile={isMobile}/>
                      <Row label="Taux de paiement"       value={`${m.payRate}%`}                                    color="#38bdf8" isMobile={isMobile}/>
                      <Row label="Objectif"               value={obj>0?`${obj.toFixed(0)} TND`:'—'}                  color="#a78bfa" isMobile={isMobile}/>
                      <Row label="Atteinte objectif"      value={obj>0?`${pct}%`:'—'}                                color={pct>=100?'#12b76a':pct>=50?'#f79009':'#ef4444'} isMobile={isMobile}/>
                      <Sep label="Stats" isMobile={isMobile}/>
                      <Row label="Commission Max"         value={m.commMax>0?`${m.commMax.toFixed(0)} TND`:'0 TND'}  color="#f79009" isMobile={isMobile}/>
                      <Row label="Commission Min"         value={m.commMin>0?`${m.commMin.toFixed(0)} TND`:'0 TND'}  color="#fb7185" isMobile={isMobile}/>
                      <Row label="Meilleur mois"          value={bestMth}                                             color="#facc15" isMobile={isMobile}/>
                    </div>
                  </div>
                </div>
              ))}
